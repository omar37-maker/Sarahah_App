...
export const registerService = async (body) => {
  ...
  const checkEmailDuplication = await UserRepository.findOneDocument(
    { email },
    { email: 1 },
  );
  if (checkEmailDuplication) {
    throw new ConflictException("email already exists", 409, "EMAIL_EXISTS", {
...
export const refreshTokenService = async (header) => {
  ...
  const { decodeToken } = await decodeToken({
    token: refreshToken,
